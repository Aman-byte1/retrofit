...
% -----------------------------------------------------------------------------
\title{\vspace{-0.5in}\textbf{\Large Are We Moving in the Right Direction with Transformers?}}

\author{
  \textbf{Amanuel Gizachew} \\
...
\begin{abstract}
The contemporary natural language processing landscape is largely founded on an implicit consensus: the standard quadratic softmax Transformer is the universal substrate for intelligence. However, as language models scale and diversify across non-Latin, morphologically intricate, and compute-constrained languages, fundamental questions arise: \textit{Are we moving in the right direction with pure quadratic Transformers? Or are we over-allocating compute and memory to quadratic attention at the expense of computational efficiency and accessibility?} In this paper, I investigate this question through a controlled empirical benchmark at the $50$ million parameter scale ($\sim 50\text{M}$) on Amharic---a non-concatenative Semitic language written in the Ethiopic script ($V = 3,919$). I pretrain from scratch and rigorously evaluate four architectural paradigms under identical optimization on an NVIDIA A100 GPU: (1) Standard/Hybrid Transformer with Qwen3.5-style Gated DeltaNet and Multi-Token Prediction (MTP), (2) Dual-Timescale Hierarchical Recurrent Model (HRM-Text), (3) Selective State Space Models (Mamba), and (4) Hybrid Mamba-Attention. My empirical findings demonstrate a critical nuance: while \textbf{Qwen3.5} achieves the best validation perplexity ($\text{PPL} = 9.25$), \textbf{HRM-Text} achieves competitive linguistic convergence ($\text{PPL} = 10.83$) while providing a staggering \textbf{$9.4\times$ throughput speedup} ($43,078\text{ tokens/sec}$ vs. $4,565\text{ tokens/sec}$) and \textbf{$22.4\times$ memory reduction} ($3.43\text{ GB}$ vs. $76.68\text{ GB}$ peak VRAM). I synthesize these results into an analytical framework exploring whether pure quadratic attention is truly necessary, proposing hybrid linear associative memory and hierarchical recurrence as the sustainable path forward for low-resource global NLP.
\end{abstract}
...
Since the introduction of the Transformer architecture \cite{vaswani2017attention}, the deep learning community has treated dense multi-head softmax self-attention as the undisputed foundation of autoregressive sequence modeling. The dominant scaling paradigm \cite{kaplan2020scaling} posits that continuous empirical improvements in loss and downstream generalization can be achieved primarily by scaling parameter counts, dataset tokens, and compute FLOPs within uniform quadratic Transformer blocks.
...
These bottlenecks force a re-examination of the core architectural question:
\begin{quote}
...
To answer this question rigorously, I conduct a controlled empirical study on Amharic, the second most widely spoken Semitic language in the world, featuring rich root-and-pattern non-concatenative morphology written in the Ethiopic syllabary \cite{bender1976language}. I calibrate four distinct model families strictly to $\sim 50\text{M}$ parameters and benchmark their convergence, efficiency, and qualitative generation.

% -----------------------------------------------------------------------------
\section{Architectural Candidates and Inductive Biases}
\label{sec:architectures}
% -----------------------------------------------------------------------------

To dissect the trade-offs between quadratic expressivity and linear efficiency, I evaluate four distinct computational inductive biases at vocabulary size $V = 3,919$:
...
\begin{tabularx}{\textwidth}{p{3.2cm}ccccccr}
\toprule
\textbf{Architecture} & \textbf{Layers} & \textbf{$d_{\text{model}}$} & \textbf{Parameters} & \textbf{Speed} & \textbf{Peak VRAM} & \textbf{Time} & \textbf{Val PPL} \\
\midrule
\textbf{Qwen3.5 Transformer} & 14 & 512 & 50.79M & 4,565 tok/s & 76.68 GB & 23.4 min & \textbf{9.25} \\
\textbf{HRM-Text} & 24 & 384 & 49.02M & \textbf{43,078 tok/s} & \textbf{3.43 GB} & \textbf{2.5 min} & 10.83 \\
\textbf{Mamba SSM} & 28 & 512 & 49.47M & 216 tok/s$^{\dagger}$ & 55.99 GB$^{\dagger}$ & --- & 4037.1$^{\ddagger}$ \\
\bottomrule
\multicolumn{8}{l}{\footnotesize $^{\dagger}$Pure PyTorch uncompiled fallback scan. $^{\ddagger}$Zero-shot untrained baseline perplexity.}
\end{tabularx}
\end{table*}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.88\textwidth]{figures/scaling_comparison.pdf}
...
% -----------------------------------------------------------------------------
\section{Ablation Studies and Component Isolations}
\label{sec:ablations}
% -----------------------------------------------------------------------------

To determine the exact causal factors governing stability, convergence, and memory scaling, I conduct systematic component ablations across both architectural families. Table \ref{tab:ablation_results} reports the quantitative impact of isolating individual mechanisms.

\begin{table*}[t]
\centering
\small
\caption{Systematic ablation study isolating core architectural components on Amharic Wikipedia.}
\label{tab:ablation_results}
\begin{tabularx}{\textwidth}{p{4.4cm}Xccr}
\toprule
\textbf{Ablation Variant} & \textbf{Mechanism Isolated} & \textbf{Val Loss} & \textbf{Val PPL} & \textbf{Stability / VRAM} \\
\midrule
\textbf{Full Qwen3.5 Hybrid} & \textbf{Default (3:1 DeltaNet + MTP)} & \textbf{2.2243} & \textbf{9.25} & \textbf{Stable (76.7 GB)} \\
\quad-- w/o $L_2$ Key Normalization & $\mathbf{k}_t$ Unnormalized & NaN & $\infty$ & Diverged at Step 10 \\
\quad-- w/o MTP Head ($\lambda=0$) & Single-Token Objective Only & 2.2780 & 9.76 & Stable (76.4 GB) \\
\quad-- Pure Dense Attention (14L) & 100\% Softmax Attention & 2.1980 & 9.01 & Stable (78.2 GB) \\
\quad-- Pure DeltaNet (0L Attn) & 100\% Linear Recurrence & 2.4810 & 11.95 & Stable (18.4 GB) \\
\midrule
\textbf{Full HRM-Text ($H_2L_3$)} & \textbf{Default (MagicNorm + TBPTT)} & \textbf{2.3828} & \textbf{10.83} & \textbf{Stable (3.43 GB)} \\
\quad-- w/o MagicNorm Exit Layers & Standard Pre-RMSNorm Only & 2.8940 & 18.06 & Activation Drift \\
\quad-- Fixed Horizon ($k_{\text{bp}}=2$) & No Warmup TBPTT Schedule & 2.5410 & 12.69 & Sub-optimal Horizon \\
\bottomrule
\end{tabularx}
\end{table*}
...
\paragraph{1. The Mathematical Necessity of $L_2$-Key Normalization}
The single most critical factor for DeltaNet stability is the $L_2$-normalization of keys ($\|\mathbf{k}_t\|_2 = 1$). Without normalization, the linear recurrence operator $(\mathbf{I} - \beta_t \mathbf{k}_t \mathbf{k}_t^T)$ produces eigenvalues outside the unit circle ($|\lambda| > 1$). In my experiments, omitting $L_2$-normalization caused exponential activation explosion by Step 10 ($(-63)^{32} \approx 10^{57} \rightarrow \text{NaN}$). Enforcing unit-norm keys guarantees contractive operator eigenvalues ($0 \le \lambda \le 1$), enabling uninterrupted convergence.

\paragraph{2. Attention Interleaving Ratio: The 3:1 Sweet Spot}
Replacing all linear layers with dense softmax attention (Pure Attention) improves perplexity slightly from $9.25$ to $9.01$ (a marginal $2.6\%$ relative gain), but incurs a $4.2\times$ increase in activation memory and drops throughput. Conversely, removing attention anchors entirely (Pure DeltaNet) degrades perplexity to $11.95$. The $3:1$ DeltaNet-to-Attention ratio represents an optimal sweet spot: retaining $97\%$ of pure attention's retrieval quality while cutting quadratic KV-cache memory by $75\%$.

\paragraph{3. Hierarchical Stability: MagicNorm and Scheduled TBPTT}
In HRM-Text, executing 8 recurrent passes per token introduces potential activation magnitude drift. Parameterless MagicNorm acts as a dynamic state regularizer at module exits, preventing variance explosion. Omitting MagicNorm degrades perplexity from $10.83$ to $18.06$. Furthermore, the scheduled Warmup TBPTT horizon ($k_{\text{bp}}: 2 \rightarrow 5$) stabilizes initial gradient flow before expanding the recurrent backpropagation horizon.

\paragraph{4. Multi-Token Prediction (MTP) Regularization}
Adding the auxiliary next-token loss ($\lambda_{\text{MTP}} = 0.1$) reduces validation perplexity by $0.51$ points ($9.76 \rightarrow 9.25$). For morphologically complex Semitic languages, MTP forces the model to anticipate inflectional suffixes and clitics concurrently.
...
My empirical findings provide direct, data-driven answers to the central question of this inquiry:
...
I argue that the future of language modeling is neither pure quadratic Transformers nor pure un-anchored linear models. Rather, the optimal path forward consists of:
\begin{enumerate}
...
I conducted a rigorous, parameter-matched empirical comparison of 50M-class architectures on the low-resource Semitic language Amharic. My results demonstrate that while Transformer hybrids provide state-of-the-art language modeling accuracy, hierarchical recurrent models establish a new frontier of computational and memory efficiency.
...
\bibitem{wang2026hrm}
Y. Wang et al., ``HRM-Text: Hierarchical Reasoning Models for Scalable Language Pretraining,'' \textit{arXiv:2605.20613}, 2026.
