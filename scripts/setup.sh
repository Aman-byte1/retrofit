...
# 1. Install base dependencies & build tools
echo "[1/3] Installing base dependencies & build tools..."
pip install -q datasets numpy matplotlib seaborn pandas ninja packaging huggingface_hub

# 2. Try to install mamba-ssm (CUDA fused kernels)
echo "[2/3] Installing Mamba SSM (CUDA fused kernels)..."
pip install --no-build-isolation causal-conv1d mamba-ssm 2>/dev/null || \
pip install causal-conv1d mamba-ssm 2>/dev/null || \
echo "  Note: mamba-ssm binary build skipped (will use pure PyTorch fallback with torch.compile)"
...
if [ ! -f "data/tokenized/train.bin" ]; then
    ...
    echo "[3/3] Tokenized Amharic dataset already present."
fi
