...
Interleaves Mamba SSM layers with Transformer self-attention layers.
Gets the best of both worlds:
- Mamba layers: O(N) linear complexity for most processing
- Attention layers: O(N²) but with strong associative recall
...
from .transformer_lm import RMSNorm, SwiGLU, TransformerAttention
...
class HybridMambaLayer(nn.Module):
    ...
    def __init__(self, d_model: int, d_state: int = 16, d_conv: int = 4, expand: int = 2):
        ...
        self.ffn = SwiGLU(d_model, d_model * 2)
...
class HybridAttentionLayer(nn.Module):
    """Pre-norm Transformer attention layer for the hybrid model."""
...
class HybridMambaTransformerLM(nn.Module):
    ...
    Architecture:
        Embedding → [Mamba, Mamba, Attn, Mamba, Mamba, Attn, ...] → RMSNorm → LM Head
    
    Every `attn_every` layers is a Transformer attention layer;
    all other layers are Mamba SSM layers.
    
    Default: attention every 3rd layer (ratio: 2:1 Mamba:Attention)
    """
...
def create_hybrid(vocab_size: int, target_params: int = 50_000_000) -> HybridMambaTransformerLM:
    """Create a Hybrid Mamba-Transformer LM targeting approximately `target_params` parameters."""
    # 18 layers (12 Mamba + 6 Attention), d=512
    model = HybridMambaTransformerLM(
